...
void Grid::Tile::setTileTexture(sf::RectangleShape& sprite, Tile tile)
{
    auto textureRectPtr { Globals::Texture::getTileTextureRect(tile) };
    if (textureRectPtr)
    {
        sprite.setTexture(&Globals::Texture::ATLAS);
        sprite.setTextureRect(*textureRectPtr);
    }
}
...
Grid::Grid::Grid(int width, int height, const std::vector<std::vector<Tile::Tile>>& tiles)
    ...
    , m_tiles { tiles }
{
    // m_background.setTexture(&Globals::Texture::BACKGROUND);
...
void Grid::Grid::draw(sf::RenderWindow& window)
{
    for (int i {}; i < m_width; ++i) for (int j {}; j < m_height; ++j)
    {
        ...
        setTileTexture(tileSprite, m_tiles[i][j]);
